...
void lgGUI::Propiedades(bool *p, Ogre::RenderWindow *win, Ogre::Entity *ent, Ogre::String nombre)
{
    ...
    if (seleccion != "luz")
    {
        if (nombre != "no")
        {
            ...
            //Rotacion
            ImGui::Text("Orientacion");
            //Ogre::Quaternion s = l->getParentNode()->getOrientation();
            static int r_p, r_y, r_r;

            ImGui::InputInt("Pitch", &r_p);
            ImGui::InputInt("Yaw", &r_y);
            ImGui::InputInt("Roll", &r_r);

            if(ImGui::Button("Aplicar"))
            {
                ent->getParentNode()->pitch(Ogre::Degree(r_p));
                ent->getParentNode()->yaw(Ogre::Degree(r_y));
                ent->getParentNode()->roll(Ogre::Degree(r_r));
            }
...
void lgGUI::PropiedadesLuz(bool *p, Ogre::RenderWindow *win, Ogre::Light *l, Ogre::String nombre)
{
    ...
    if (seleccion == "luz")
    {
        ...
        if (nombre != "no")
        {
            ...
            ImGui::Text("Orientacion");
            Ogre::Quaternion s = l->getParentNode()->getOrientation();
            std::cout<<"pitch : "<<s.getPitch().valueDegrees()<<std::endl;
            std::cout<<"yaw : "<<s.getYaw().valueDegrees()<<std::endl;
            std::cout<<"roll : "<<s.getRoll().valueDegrees()<<std::endl;
            static int r_p, r_y, r_r;
            ...
            if(ImGui::Button("Aplicar"))
            {
                l->getParentNode()->pitch(Ogre::Degree(r_p));
                l->getParentNode()->yaw(Ogre::Degree(r_y));
                l->getParentNode()->roll(Ogre::Degree(r_r));
            }
            ///
            //asignar material
            ImGui::BeginChild("Color", ImVec2(250, 0), true);
            ...
            ImGui::EndChild();
            ImGui::Separator();
            int tipo_luz_n = l->getType();

            ImGui::Text("Tipo de luz");

            const char *tipo_luz[] = {"Punto", "Direccional", "Foco"};
            int luz_actual = tipo_luz_n;
