...
void lgGUI::MenuBar(bool *p_open, Ogre::SceneManager* sm)
{
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_NoTitleBar;
    window_flags |= ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove;
    window_flags |= ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoNavFocus;

    static bool abrir_material;
    if (abrir_material == true) archivos(&abrir_material,".material", sm);
    if (abrir_mesh == true) archivos(&abrir_mesh,".mesh", sm);
    if (agregar_luz == true) agregarLuzGUI(&agregar_luz,"luz", sm);
    if (guardar_xml == true) guardar(&guardar_xml,sm);

    if (ImGui::BeginMainMenuBar())
    {
        if (ImGui::BeginMenu("Archivo"))
        {
            if(ImGui::MenuItem("Guardar")) guardar_xml = true;
            ImGui::EndMenu();
        }
        if (ImGui::BeginMenu("Edicion"))
        {
            if (ImGui::MenuItem("Abrir Material", "Selecciona un material"))
            {
                abrir_material = true;
            }
            ImGui::EndMenu();
        ...
        {
            if (ImGui::MenuItem("Mesh", "Selecciona un mesh"))
            {
                abrir_mesh = true;
            }
            if (ImGui::MenuItem("Luz"))
            {
                agregar_luz = true;
            }
            ImGui::EndMenu();
        }
        tmenubar = ImGui::GetWindowHeight();
